 ...
 *  is the gh-direct issue plan (`/__vow/issues`); `events` is the live event stream (`/__vow/events`);
 *  `agent` is the start-work signal that dispatches an agent session for an issue (`/__vow/agent`). Both the
