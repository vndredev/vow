...
const EXPECTED_BUTTON = [
  ...
  ``,
  `const props = withDefaults(defineProps<{ label?: string; icon?: IconName; variant?: 'default' | 'outline' | 'ghost'; size?: 'sm' | 'md' | 'lg'; type?: 'button' | 'submit' }>(), { label: '', variant: 'default', size: 'md', type: 'button' });`,
  `</script>`,
  ``,
  `<template>`,
