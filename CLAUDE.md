...
- **Generation** (what vow emits): DONE so far — entity/view/**form**/bind, field types (text/**longtext**/number/boolean/select/date/**reference**) + a stable auto-id + per-entity **zod** schemas (`create<Name>` is `.parse`), views as YAML `## view` (semantic blocks + the primitive/`link:` vocabulary + layout primitives incl. **Stack** + tokens), **ten primitives** (checkbox/collapsible/tabs/dialog/select/switch/radio-group/button/field/**badge**; button + badge carry token-driven `variant` systems), **icons in views** (an `icon:` node + an `icon` on Button/Link, from the swappable `@vow/icons`), **`emit form`** (a bound, validated `## form` of `<Field>`s with per-field errors), **multi-view nav from vows** (non-root views + forms → `/<slug>` routes + a generated nav), **`@vow/store`** (in-memory data adapter), **`@vow/shell`** (dashboard-sidebar app chrome), the **docs system** (`@vow/markdown`→`@vow/docs`; the docs are a generated vow app), `@vow/router` (boot-level, hash anchors, 404, title), docs chrome (⌘K search + mobile drawer on `dialog`, sidebar, TOC, dark toggle, Inter), `@vow/icons` (Lucide). NEXT → referent-display-resolution (show a reference's target name, not the id) → more view patterns (table/cards/board/stats) → primitive ladder cont'd (table; wrap complex via Zag/Ark) → data adapter (memory → CF D1).
