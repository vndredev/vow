...
**The realtime-observability stream** (the next arc, #497). The hub records a live event feed — `run.started` / `run.phase` / `run.finished` / `pr.merged` today — to `.vow/events.jsonl`; [`vow events`](/guide/cli#realtime-observability) prints the trace. Two consumers read it: an **orchestrator** (a Claude Code session connected via [Channels](https://code.claude.com/docs/en/channels.md)) that acts on observed state without being told, and the **studio panel** (the control + 100%-trace, on/off — coming). The channel is observability: you watch, you don't narrate.
