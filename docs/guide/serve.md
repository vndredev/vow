...
3. **The studio reads it** through the `useAgentLoopStatus()` hook in `@vow/store`, polled on focus + a 5s interval (mirroring `usePlan` / `useEvents`). A malformed response degrades to the same idle default — the surface is read-only and never throws.
...
- **The agent loop** — `loop: { as: status }`, a generated view element (`VowAgentLoopStatus`) bound to the `useAgentLoopStatus()` hook. It **composes vow primitives** — a [`Badge`](/guide/primitives/badge) for the run state (a positive tone when **running**, neutral when **idle**) above a [`Stats`](/guide/primitives/stats) stat-card grid for the round's `round` / `backlog` / `openPrs` metrics (plus the optional last-round) — so it inherits the design language exactly like the plan views, not bespoke classed markup. The [agent-loop status](#the-agent-loop-status) made visible. Read-only: the loop is observed here, never driven from this element.
...
3. ✅ **The agent watch-loop** — `vow serve --watch` runs the self-heal loop in the hub daemon, **opt-in only** via `--yes` / `VOW_AGENT_AUTO=1` (the [#486](/guide/agent) gate): a spiral runs, then re-runs every 60s, developing the [ready-queue](/guide/agent#the-ready-queue-the-session-lifecycle) (the right next work, dependency-aware) and merging green; the daemon stays up between spirals, the inter-spiral wait abortable for a prompt shutdown. Default (no `--watch`) is off. On startup it **prunes a prior run's stale worktrees** (`.vow-worktrees/feat-issue-N`) so a restart never hits "branch already used by worktree" — an in-flight run's worktree (an issue with an open PR) is spared. Each round develops a batch **partitioned by area** — at most one issue per `area:` label — so concurrent develops touch disjoint files and converged work merges instead of [drafting on a rebase conflict](/guide/agent#partitioned-rounds-one-issue-per-area-so-converged-work-merges-instead-of-drafting). **A 60s tick on a clean, unchanged tree is a no-op**: a findings-free audit stamps the HEAD SHA to `.vow/last-clean-audit`; the next tick reads `HEAD === stamp` and returns `done` immediately, with zero Fable spend.
