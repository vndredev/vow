...
`deriveStatus` (`@vow/core`) reads a vow's status straight off this truth: `done` when every claim it promises is covered, `active` when some is, `planned` when none is yet — and a parent rolls its children up. So the [roadmap](/guide/roadmap) can derive itself rather than be hand-maintained (`blocked` joins once CI reports a failing test).
