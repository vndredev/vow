...
### Operating the plan — close & reopen in the studio

The plan isn't only read in the UI. Each issue view (table · board · roadmap) carries a **Close / Reopen** button: it `POST`s `/__vow/issues` (`{ action, number }`), and the dev plugin calls the **same `@vow/observability` `closeIssue`/`reopenIssue` the MCP's `close_issue` does** — one seam for a person and the agent. The reply is the freshly re-shelled plan, so the studio reflects the new derived status (closed → done) at once. _Foundation:_ this is dev-only (the plugin shells `gh`); in prod a Worker serves the same write over the GitHub API. Assigning and opening new issues stay agent-side (the MCP) for now.

### Starting work — the board signals the agent

Beside Close / Reopen, every issue not yet `done` carries a **Start work** button — the human's one signal to begin. It `POST`s `/__vow/agent` (`{ action: "start", number }`); the dev plugin resolves the issue (its number/title/body — the spec injected into the session) and dispatches **`vow agent run <n>`** for it, detached, then replies `202 Accepted` with the dispatched issue. The run develops the issue in an isolated worktree and opens a PR — and that PR is what derives the issue to `doing`. **Status stays derived** — the signal only begins the run; nothing is written to the issue's status by hand. This is the channel from the studio into the agent (#96, #97), reusing the same `vow agent` CLI a person runs by hand. _Foundation:_ dev-only (the plugin spawns the local `vow` CLI). The production channel (a Worker over the GitHub API) and the real MCP-notification surface (a research-preview API) are the deferred next step.
