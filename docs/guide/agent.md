...
It maps the **known vow-banned rules** to their remedy — the oxlint quality wall (`no-ternary` → an if/else block, `no-negated-condition` → lead with the positive branch, `no-undefined` → restructure to a definite value (or the `Maybe<T>` seam, which needs `@vow/core`; `null` is also banned), `no-explicit-any` / the `as` cast → narrow with a type predicate, `no-magic-numbers`, `sort-keys`, `sort-imports` → reorder imports by hand (multiple-specifier before single, then alphabetical; `vp fmt` won't), `require-await` → drop the `async` or add the `await`, `max-lines` → split by concern, `capitalized-comments` → start the comment with a capital, `no-inline-comments` → its own line, the **OS-adapter-boundary** escapes for Node's inherently-mutable APIs — `prefer-readonly-parameter-types` → a targeted disable on a stream/`EventEmitter`/`Buffer` param, `no-duplicate-imports` / `consistent-type-specifier-style` → the inline `import { type X, value }` + a file-top disable for a value+type import, the sanctioned `mcp/channel.ts` patterns), the **strict-tsgo resolution errors** (`Cannot find name` → import / declare / destructure from the builder's return, `Cannot find module` / `Cannot find package '@vow/*'` → declare the cross-package dep as `workspace:*`), and the vow gates (framework-neutrality → the neutral `@vow/component` model, provider-neutrality → behind a `Provider` adapter, design-language coverage → a vow.css token, `no-cycle` → invert the dependency, has-a-doc → update the page). A rule it doesn't know about **passes through verbatim** — the correction is never lossy. These tsgo entries are the ones that stalled live develop rounds: an undeclared `@vow/core` import and a bare `sfc` in a generated-SFC test got the raw error but no rewrite, so the fix-rounds never converged.
...
A fix-round must be **fast** — re-running the whole-repo `pnpm -r test` on every iteration once ground a single fix for 35+ minutes (#676). So the loop runs two gate sets. Each develop pass and each **fix-round** re-runs the **fast** set — `vp lint` (whole-repo lint is quick) plus the **touched package's** tests (`vp test <package>`, scoped by the issue's `area:` label), never the whole-repo suite. The fix rounds are also wall-time-bounded: after a fixed budget (or the attempt cap), a still-red run drafts rather than hangs. Only when the fast rounds converge green does the loop run the **thorough** final verify **once** — `vp check` (the full lint + typecheck + format) plus the **touched package's** tests — and that verdict decides merge vs. draft.

The final verify is **worktree-safe** by construction: it does **not** run the whole-repo `pnpm -r test`. The loop develops each issue in a git **worktree**, where a worktree's `.git` is a _file_, not a directory — so `pnpm -r test` (its `@vow/cli` agent-auto/audit tests and the `vp test` path-placeholder expansion need a real repo) fails with `fatal: not a git repository`. That whole-repo failure drafted **every** run, so nothing ever converged (#685). The fix scopes the final verify to `vp check` + the touched package, exactly like the fast `fixGates` but with `vp check` in place of `vp lint`. **CI is the full-suite backstop**: a green worktree-local final verify opens a **non-draft** PR, CI runs `pnpm -r test` in a real-repo checkout, and the per-PR settle merges on green — so the full coverage is never lost, and a narrow worktree-local verify can never land an untested change.
...
## Partitioned rounds — one issue per area, so converged work merges instead of drafting

Decoupling the settle made the loop fast for **disjoint** work, but a concurrent batch that touched **overlapping files** still drafted: three round-2 issues all in the `agent`/`mcp` layer developed off **one** base, so when the per-PR settle rebased each onto the moved `main` they **conflicted** and got drafted — needing manual resolution (#681). The fix is to **partition the concurrent batch by area**: each round picks **at most one issue per `area:` label** (`partitionByArea`, resolving each issue's area via `areaOf` from a single batched `gh issue list --json number,labels`). So the round's concurrent develops touch **disjoint files** and the per-PR settle merges them one area at a time instead of fleet-conflicting. Same-area siblings simply **wait for the next round** — developed once their area is free (a cleanly-merged area is free the next tick). Area-**less** issues (no `area:` label) are never collapsed together: each touches an unknown surface, so each still develops. This trades a little parallelism for **conflict-free merges** — correctness over raw speed.

## Stale worktrees — pruned on restart, so a branch is never blocked

A run develops each issue in `.vow-worktrees/feat-issue-N`. If the loop is restarted while leftovers remain, the new round's `git worktree add -B feat/issue-N` hits **"branch already used by worktree"** and can't reuse the branch (#681). So `vow serve --watch` **prunes stale worktrees on startup**, before the first spiral: `cleanStaleWorktrees(cwd, active)` lists the `feat-issue-N` dirs, keeps any whose issue is still **active** (an open PR — its worktree may be live), and force-removes the rest (`git worktree remove --force` + `git worktree prune`). An in-flight run's worktree is **spared**, a non-`feat-issue-N` directory is left untouched, and each removal is best-effort — a cleanup hiccup never blocks the hub coming up. The staleness **decision** is pure (`staleWorktrees` in `@vow/agent`); the fs + git shelling is its `@vow/cli` wrapper.

## The ready-queue + the session lifecycle

The loop no longer develops the **oldest open issue** — it pulls the **right next work** from the local plan's ready-queue (#740). Each round, before developing, it: **syncs** the GitHub issues into the plan (`vow plan sync` — open → `backlog`, closed → `done`); **reconciles** stale claims against the live PRs (a `doing` item whose issue has no in-flight PR is an orphan, released back to `ready`); **auto-readies** the unblocked backlog (a `backlog` item whose every dependency is done → `ready`); then develops the **ready-queue** — the `ready`, unblocked items in priority order (`readyQueue` / `promotable` in `@vow/plan`, the gh wrapper in `@vow/cli`'s `plan-ops`). A blocked item **waits** for its dependency to merge; a prioritized one leads — the loop is dependency-aware, not date-ordered.

And it **owns the in-flight state**: claiming an item opens a local **`plan_session`** (its branch + worktree) and transitions it `ready → doing`. vow's own lifecycle (`backlog → ready → doing → review → done`, with `doing → ready` the release) is the source of truth — reconciled against reality every round — so the **stale-claim bug ([#623](https://github.com/vndredev/vow/issues/623)) is structurally impossible**: an orphaned claim can't stick, because the session is released the moment its PR is gone, not left on a GitHub label that orphans. The claim lives entirely in the local plan — no GitHub label or Project mirror ([#741](https://github.com/vndredev/vow/issues/741) retired both).
...
✓ provider abstraction → ✓ plan-builder → ✓ dispatch → ✓ verify + PR → ✓ `runTask` (the loop, one call) → ✓ `realOps` (the real exec — git worktrees + the CLI spawned via `spawn`; each run is its own tracked process, so a parallel fleet never blocks, and SIGTERM/SIGINT kill all in-flight agents on shutdown — no orphaned `claude -p` processes after a restart (#683)) → ✓ `vow agent run <n>` + `run-all <n>...` (the CLI front-door — develops issues in worktrees and opens PRs, draft if gates fail; live-streaming progress; auth choices; NDJSON for LLMs) → ✓ `vow agent merge` (the agent-merge stage — polls CI's `gate`, merges a green PR squash+delete-branch, drafts a red one) → ✓ `vow agent auto --yes` (the self-heal loop — **opt-in only**: it audits + develops + merges unsupervised, so it refuses to start without `--yes` / `VOW_AGENT_AUTO=1`, and `--help` is help, never a launch) → ✓ `vow agent audit` → ✓ the **trigger** (the `/__vow/agent` seam dispatches `vow agent run <n>` for an issue — the loop's lever to begin; the run's PR derives `doing`) → ✓ **team dispatch** (the loop routes each issue by its `area:` label to the matching [team](#the-elite-team) specialist and injects that agent's complete brief into the develop plan — one source of agent definitions, #638) → ✓ **spec-compliance review** (a fresh headless reviewer checks "right code?" BEFORE the quality gates — closes the "green but wrong" blind spot; #648) → next: the production channel (a Worker over the GitHub API) + the real MCP-notification surface. The whole thing is vow's own, provider-neutral — not a dependency on any single CLI's orchestration.
