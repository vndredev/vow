...
## Orphan agents — killed on shutdown, so a restart starts clean

Prior to #683, `vow serve --watch` spawned `claude -p` via `execFileSync`, which **blocks the Node.js event loop** for the duration of the agent run (20–38 min observed). With the event loop blocked, SIGTERM/SIGINT handlers could not fire until the child finished — and once the parent exited, the `claude -p` processes became **orphans** that kept running. On a restart, a surviving orphan could collide with the new loop's agent on the **same branch** (`feat/issue-N`), clobbering commits.

The fix is two-part:

1. **`execText` uses `spawn` (async) instead of `execFileSync` (sync).** With `spawn`, the Node.js event loop stays alive while the child runs. Signal handlers fire promptly: when SIGTERM arrives at `vow serve`, the hub's handler fires immediately instead of waiting for a 20-min child to finish.

2. **A PID registry + `killRunningAgents`.** `execText` adds each spawned child's PID to a module-level `agentPids: Set<number>` before the child starts, and removes it when the `close` event fires. `killRunningAgents()` (exported from `@vow/agent`) sends SIGTERM to every tracked PID — best-effort, silently skipping PIDs that are already gone. `shutDown()` in `vow serve` calls `killRunningAgents()` **before** aborting the loops, so a controlled restart or power-down terminates the children first.

Together with the startup worktree prune (#681), this closes the "clean-restart gap": a restarted hub has no leftover branches AND no leftover processes.
