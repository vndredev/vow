...
Card is structural — a styled `<div>`, no `@vow/headless` logic. The [`cards` view pattern](/guide/views) composes these per record; like the entity list, that is a **composition**, not a primitive. See [Architecture](/guide/architecture).
